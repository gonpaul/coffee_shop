...
class CoffeeOrderBuilder:
    """
    CoffeeOrderBuilder: Builder для создания заказа кофе с fluent интерфейсом.

    Правила и ограничения:
      - base (str) и size (str) обязательны для build()
      - milk (str): "none", "whole", "skim", "oat", "soy" (по умолчанию: "none")
      - syrups: не более 4, дубликаты игнорируются, порядок добавления сохраняется (по умолчанию: пусто)
      - sugar (int): 0-5 чайных ложек (по умолчанию: 0)
      - iced (bool): True/False (по умолчанию: False)
    ...
    Методы:
      - set_base(base: str) -> CoffeeOrderBuilder
      - set_size(size: str) -> CoffeeOrderBuilder
      - set_milk(milk: str = "none") -> CoffeeOrderBuilder
      - add_syrup(name: str) -> CoffeeOrderBuilder
      - set_sugar(teaspoons: int = 0) -> CoffeeOrderBuilder
      - set_iced(iced: bool = False) -> CoffeeOrderBuilder
      - clear_extras() -> CoffeeOrderBuilder
      - build() -> CoffeeOrder
    """
    # Базовые цены
    BASE_PRICES = {
    ...
    def set_base(self, base: str) -> "CoffeeOrderBuilder":
        """
        Задать кофейную базу (espresso, americano, latte, cappuccino).
        :param base: str
        :return: self
        """
        self.base = base
    ...
    def set_size(self, size: str) -> "CoffeeOrderBuilder":
        """
        Задать размер (small, medium, large).
        :param size: str
        :return: self
        """
        self.size = size
    ...
    def set_milk(self, milk: str = "none") -> "CoffeeOrderBuilder":
        """
        Задать тип молока. По умолчанию "none".
        :param milk: str
        :return: self
        """
        self.milk = milk
    ...
    def add_syrup(self, name: str) -> "CoffeeOrderBuilder":
        """
        Добавить сироп (до 4 шт). Дубликаты игнорируются.
        :param name: str
        :return: self
        """
    ...
    def set_sugar(self, teaspoons: int = 0) -> "CoffeeOrderBuilder":
        """
        Задать количество сахара (0..5). По умолчанию 0.
        :param teaspoons: int
        :return: self
        :raises ValueError: если за пределами диапазона
        """
    ...
    def set_iced(self, iced: bool = False) -> "CoffeeOrderBuilder":
        """
        Сделать напиток холодным (iced). По умолчанию False.
        :param iced: bool
        :return: self
        """
        self.iced = iced
    ...
    def build(self) -> CoffeeOrder:
        """
        Собрать заказ CoffeeOrder.
        :return: CoffeeOrder
        :raises ValueError: если не заданы base или size
        """
        if not self.base:
